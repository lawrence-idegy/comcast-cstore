...
export const LocationNavBar = ({ currentRoomId, onNavigate, isHidden = false, animateIn = false }: LocationNavBarProps) => {
  ...
  // Trigger rise animation after mount when animateIn is true
  useEffect(() => {
    ...
    <>
      ...
      <div
        className={cn(
          ...
          isHidden ? 'opacity-0 pointer-events-none translate-y-4 duration-300' : 'opacity-100 translate-y-0',
          // Rise from bottom animation
          animateIn && !hasAnimatedIn ? 'translate-y-24 opacity-0 duration-700' : 'duration-700'
        )}
      >
